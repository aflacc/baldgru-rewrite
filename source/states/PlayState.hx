...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		stagesFunc(function(stage:BaseStage) stage.createPreHUD()); // Thank you bullshit psych code
		...
		callOnScripts('onCreatePost'); // thgsbullshit
		stagesFunc(function(stage:BaseStage) stage.createPost());
	...
	{
		if (!videoPlaying) // causes problems
		{
			FlxG.camera.followLerp = 0;
		...
		if (ret != LuaUtils.Function_Stop && !transitioning)
		{
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					{
						StoryMenuState.weekCompleted.set("story", true);
						Highscore.saveWeekScore("story", campaignScore, 0);
			...
			else
			{
				...
				if (SONG.song.toLowerCase() == "yolked")
				{
					ClientPrefs.data.defeatedGeremy = true;
					ClientPrefs.saveSettings();
					// Highscore.saveScore("yolked",songScore,0,ratingPercent);
				}
