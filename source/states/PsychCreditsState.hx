	...
	override function create()
	{
		...
		var defaultList:Array<Array<String>> = [
			...
			[
				'BeefStarchJello',
				'bald/beef',
				'Odd Day Director, Charter, Voice of Bald Gru & Sonic, THE Artist for Bald Gru',
			...
			[
				'Trav',
				'bald/trav',
				'Even Day Director, Lead Musician for Bald Gru, Voice of Geremy and Sonic Do Mau',
			...
			['Scrumbo_', 'bald/scrumb', 'Composer', 'https://twitter.com/scrumbo_', 'ffbfbf'],
			[
				'Izzys_Trying',
				'bald/iz',
				'Made GameBanana Cover Art, Logo, & Album cover',
		...
		for (i in 0...creditsStuff.length)
		{
			...
			if (isSelectable)
			{
				...
				/*if (creditsStuff[i][1].startsWith(""))
				{
					/*var string = creditsStuff[i][1].split("/")[1];
					var icon:AttachedSprite = new AttachedSprite(str);
					icon.frames = Paths.getSparrowAtlas(str); // Fuck
					icon.animation.addByPrefix("icon",string,24,true);
					...
					switch (string) // Funky offsetting! a whopping two (2) icons (as of right now) need different offsets!
					{
						case 'beef':
							icon.xAdd = optionText.width - 30;
							icon.yAdd = optionText.width - 580;
						case 'luigiman':
							icon.xAdd = optionText.width - 30;
							icon.yAdd = -170; // WHAT THE FUCK. BIG ASS HEAD!!! ‼
						default:
							icon.xAdd = optionText.width - 30;
							icon.yAdd = -80;
					}
				...
				{*/
					...
					switch (string) // Freaky ass offseting. LOL
					{
						case 'luigiman':
							icon.yAdd -= 90; // WHAT THE FUCK. BIG ASS HEAD!!! ‼
					}
					// using a FlxGroup is too much fuss!
					iconArray.push(icon);
					add(icon);
